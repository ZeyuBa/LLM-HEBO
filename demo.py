...
def parse_output(extracted_output):
  ...
  for param_name, b_range in bounds_dict.items():
      if 'lower_bound' in b_range and 'upper_bound' in b_range:
        lower_bound=eval(b_range['lower_bound']) if isinstance(b_range['lower_bound'],str) else b_range['lower_bound']
        upper_bound=eval(b_range['upper_bound']) if isinstance(b_range['upper_bound'],str) else b_range['upper_bound']
...
for i_rep in range(num_reps):
  ...
  old_value_pairs_with_i_step = []  # format: [([(lower_bound,upper_bound,is_log),...],valid_loss,i_step)]
